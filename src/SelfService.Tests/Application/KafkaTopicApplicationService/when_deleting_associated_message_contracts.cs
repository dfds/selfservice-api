...
public class when_deleting_associated_message_contracts : IAsyncLifetime
{
    ...
    {
        _mock.Setup(x => x.FindBy(It.IsAny<KafkaTopicId>())).ReturnsAsync(new[] { _aMessageContract });

        var sut = A.KafkaTopicApplicationService.WithMessageContractRepository(_mock.Object).Build();
