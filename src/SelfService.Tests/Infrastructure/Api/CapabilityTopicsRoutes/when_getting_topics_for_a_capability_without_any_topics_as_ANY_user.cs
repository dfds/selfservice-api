...
public class when_getting_topics_for_a_capability_without_any_topics_as_ANY_user : IAsyncLifetime
{
    ...
    [Fact]
    public async Task then_returns_expected_items()
    {
        var content = await _response.Content.ReadAsStringAsync();
        var document = JsonSerializer.Deserialize<JsonDocument>(content);

        var items = document?.SelectElements("items").ToArray();
