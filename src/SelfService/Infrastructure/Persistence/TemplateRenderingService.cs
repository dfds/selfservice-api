...
public class TemplateRenderingService : ITemplateRenderingService
{
    ...
    private static readonly VariableEntry[] Variables =
    {
        ...
        new PatternVariable("Requirement.<id>",
            ...
            FallbackOnMiss: "N/A"),
        new PatternVariable("Requirement.<id>.DisplayName",
            "Requirement display name", "Requirement", "Use of Mandatory Tags",
            new Regex(@"^Requirement\.(?<id>[^.]+)\.DisplayName$", RegexOptions.Compiled),
            (m, ctx) => ResolveRequirementDisplayName(ctx, m.Groups["id"].Value),
            FallbackOnMiss: "N/A"),
        new PatternVariable("Requirement.<id>.HelpUrl",
            "Requirement help URL", "Requirement", "https://wiki.dfds.cloud/...",
        ...
        new PatternVariable("Requirement.<unknown>", "", "Requirement", "",
            new Regex(@"^Requirement\..+$", RegexOptions.Compiled),
            (_, _) => null, FallbackOnMiss: "N/A", Hidden: true),

        new StaticVariable("Aws.AccountId", "AWS account number (12-digit)", "AwsAccount", "123456789012",
            ctx => ctx.AwsAccount?.Registration.AccountId?.ToString() ?? "N/A"),
        new StaticVariable("Aws.Status", "AWS account status", "AwsAccount", "Completed",
            ctx => ctx.AwsAccount?.Status.ToString() ?? "N/A"),
        new StaticVariable("Aws.Namespace", "Kubernetes namespace linked to AWS account", "AwsAccount", "my-capability-abc12",
            ctx => ctx.AwsAccount?.KubernetesLink.Namespace ?? "N/A"),
        new StaticVariable("Aws.RoleEmail", "AWS account role email", "AwsAccount", "aws.123456789012@dfds.com",
            ctx => ctx.AwsAccount?.Registration.RoleEmail ?? "N/A"),

        new StaticVariable("Azure.ResourceCount", "Number of Azure resource groups", "AzureResource", "2",
            ctx => ctx.AzureResources.Count.ToString()),
        new StaticVariable("Azure.Environments", "Comma-separated Azure environments", "AzureResource", "dev, prod",
            ctx => ctx.AzureResources.Count > 0
                ? string.Join(", ", ctx.AzureResources.Select(r => r.Environment).OrderBy(e => e))
                : "None"),
        new PatternVariable("Azure.<env>.Id",
            "Resource group ID for a specific environment (e.g. Azure.dev.Id)",
            "AzureResource", "a1b2c3d4-e5f6-...",
        ...
        new StaticVariable("MembershipApplications.PendingCount", "Number of pending membership applications",
        ...
        new PatternVariable("Metadata.<key>", "", "Capability", "",
            new Regex(@"^Metadata\.(?<key>.+)$", RegexOptions.Compiled),
            (m, ctx) => LookupMetadata(ctx.Capability.JsonMetadata, m.Groups["key"].Value),
            Hidden: true),
    };

    private static readonly Dictionary<string, StaticVariable> ByName =
        Variables.OfType<StaticVariable>().ToDictionary(v => v.Name);

    private static readonly PatternVariable[] Patterns =
        Variables.OfType<PatternVariable>().ToArray();
    ...
    {
        if (string.IsNullOrEmpty(jsonMetadata)) return null;
        try
        {
